...
/// Takes [surahNumber] and returns introductory tafseer text for the surah.
String getSurahInfo(int surahNumber) {
  if (surahNumber > 114 || surahNumber <= 0) {
    throw "No Surah found with given surahNumber";
  }
  return surah[surahNumber - 1]['surahInfo']?.toString() ?? '';
}

/// Takes [surahNumber] and returns the source book for [getSurahInfo].
String getSurahInfoSource(int surahNumber) {
  if (surahNumber > 114 || surahNumber <= 0) {
    throw "No Surah found with given surahNumber";
  }
  return surah[surahNumber - 1]['surahInfoFromBook']?.toString() ?? '';
}
